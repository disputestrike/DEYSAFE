...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path == "/api/safemeet/session":
            # FIELD/owner: exact session state. A raw UUID alone is not enough;
            ...
            if not row:
                return self._json({"ok": False, "error": "not found"}, 404)
            if row.get("owner_token") != owner and not self._authed():
                return self._json({"ok": False, "error": "owner_token required"}, 403)
            checkins = db.get_safemeet_checkins(sid, limit=self._limit(u, 50))
            risk = {
                "risk_level": row.get("risk_level") or "medium",
                "state": row.get("state") or "scheduled",
                "missed_checkins": row.get("missed_checkins") or 0,
            ...
            return self._json({"ok": True,
                               "session": safety.safemeet_session_public(row),
                               "checkins": [safety.safemeet_checkin_public(c) for c in checkins],
    ...
    def do_POST(self):
        ...
        if u.path == "/api/safemeet/start":
            ...
            row = db.insert_safemeet_session({
                ...
                "vehicle_description": (data.get("vehicle_description") or data.get("vehicle") or "").strip(),
                "license_plate": (data.get("license_plate") or data.get("plate") or "").strip(),
                "expected_arrival": (data.get("expected_arrival") or "").strip(),
                "expected_departure": (data.get("expected_departure") or "").strip(),
                "checkin_interval_minutes": data.get("checkin_interval_minutes") or 30,
                "user_notes": (data.get("user_notes") or "").strip(),
            })
            db.audit("field", "safemeet_start", "ref=%s place=%s risk=%s" % (
                (row.get("session_uuid") or "")[:8], place, row.get("risk_level")))
            return self._json({"ok": True,
        ...
        if u.path == "/api/safemeet/checkin":
            # FIELD: arrival/heartbeat/duress check-in. Owner token is required so a
            ...
            row = db.get_safemeet_session(sid) if sid else None
            if not row:
                return self._json({"ok": False, "error": "session not found"}, 404)
            if row.get("owner_token") != owner:
                return self._json({"ok": False, "error": "unauthorized"}, 403)
            status = (data.get("status") or data.get("checkin_type") or "manual").strip().lower()
            lat, lng = _float_or_none(data.get("lat")), _float_or_none(data.get("lng"))
            checkin = db.insert_safemeet_checkin({
                "session_id": row.get("id"),
                "checkin_type": status,
                "lat": lat,
                "lng": lng,
                "location_accuracy": _float_or_none(data.get("accuracy") or data.get("location_accuracy")),
                "battery_level": _float_or_none(data.get("battery") or data.get("battery_level")),
                "network_type": (data.get("network") or data.get("network_type") or "").strip(),
                "note": (data.get("note") or "").strip(),
                "duress_flag": status in ("duress", "duress_safe", "duress_covert"),
                "photo_url": (data.get("photo_url") or "").strip(),
            })
            updates = {}
            if status == "arrived" and row.get("state") == "scheduled":
                updates["state"] = "in_progress"
                updates["actual_arrival"] = self._now()
            if status in ("duress", "duress_safe", "duress_covert"):
                updates["state"] = "escalated"
                updates["duress_triggered"] = True
                updates["escalated_at"] = self._now()
                updates["escalation_reason"] = "SafeMeet duress check-in"
            anomalies = safety.detect_meeting_anomalies(
                row, lat, lng, (data.get("device_status") or data.get("network") or "online"))
            if anomalies.get("location_changed"):
                updates["location_changed"] = True
            if anomalies.get("route_deviation"):
                updates["route_deviation"] = True
            if anomalies.get("duration_exceeded") or anomalies.get("checkin_missed"):
                updates["escalation_reason"] = "; ".join(anomalies.get("reasons") or [])[:240]
            updated = db.update_safemeet_session(sid, updates) if updates else db.get_safemeet_session(sid)
            db.audit("field", "safemeet_checkin", "ref=%s status=%s anomalies=%s" % (
                sid[:8], status, ",".join(anomalies.get("reasons") or [])))
            return self._json({"ok": True,
                               "checkin_id": checkin.get("checkin_uuid") if checkin else None,
                               "session_status": (updated or row).get("state"),
                               "session": safety.safemeet_session_public(updated or row),
                               "anomalies": anomalies})
        ...
        if u.path == "/api/safemeet/end":
            ...
            if row.get("owner_token") != owner:
                return self._json({"ok": False, "error": "unauthorized"}, 403)
            updated = db.update_safemeet_session(sid, {
                "state": "completed",
                "actual_departure": self._now(),
            })
            db.insert_safemeet_checkin({"session_id": row.get("id"), "checkin_type": "completed",
                                        "note": "owner ended meeting"})
            db.audit("field", "safemeet_end", "ref=%s" % sid[:8])
            return self._json({"ok": True, "session": safety.safemeet_session_public(updated)})
        ...
        if u.path == "/api/safemeet/invite":
            ...
            if row.get("owner_token") != owner:
                return self._json({"ok": False, "error": "unauthorized"}, 403)
            raw_invite = "%s|%s|%s" % (sid, data.get("invite_to_token") or data.get("to") or "", self._client_id())
            invite_token = hashlib.sha256(raw_invite.encode("utf-8")).hexdigest()[:12].upper()
            db.audit("field", "safemeet_invite", "ref=%s channel=%s" % (
                sid[:8], (data.get("channel") or "direct")))
        ...
        if u.path == "/api/beacon-relay":
            ...
            if not case:
                ...
                db.audit("beacon", "relay_unknown", "receipt=%s" % receipt)
                return self._json({"ok": True, "matched": False,
                                   "receipt_ref": receipt})  # unknown beacon -> reveal nothing
            try:
        ...
        if u.path == "/api/missing":
            ...
            try:
                ...
                cnt = 1
            case_ref = db.insert_missing({"name": name, "age": (data.get("age") or "").strip(), "place": place,
