...
| **AI evidence review** — visible home image/video intake for media validation, custody facts, written-context AI/rules triage, and an explicit no-fake-vision status | user evidence ask | `index.aiMedia`/`analyzeEvidence` + `/api/media/analyze` | built for triage; computer vision adapter pending | `validate_product.py` AI evidence review + `/api/media/analyze` gate | PASS when gate green |
...
| **AI evidence-context triage** — image/video upload metadata + human-written prompt/context → suggested report fields where possible | user "where is image/video AI" | `api` `/api/media/analyze` (`media_analysis_triage`) + index `analyzeEvidence` | ✅ metadata/text triage; vision not wired | gate: endpoint returns `vision_ready=false` and next step | PASS when gate green |
