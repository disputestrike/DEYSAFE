...
| Report-burst abuse (OPEN — being addressed) | NAT-safe content-aware dup throttle in `engine/api.py` (`_duplicate_report_throttled`, `_content_signature`); per-IP request capping deliberately avoided so genuine mass-reporting during an attack is never silenced | A repeated near-identical report from one caller is 429'd while DISTINCT reports from the same carrier-grade-NAT IP always pass; final tuning + gate sign-off pending the separate gate run |
