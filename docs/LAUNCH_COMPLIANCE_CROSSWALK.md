...
| Camera/video evidence | Implemented metadata + key-gated raw upload | `app/index.html` `rMedia`, `mediaMetaFromInput`, `uploadEvidenceIfAny`, `/api/media/presign`, `/api/report`, `/api/evidence`, SHIELD case workspace | Field user can capture image/video and attach file metadata/hash to anonymous report; with Cloudflare R2 keys and bucket CORS, raw evidence uploads directly to object storage | `validate_product.py` report media metadata + R2 presign + evidence vault checks | Configure R2 env vars/CORS; add retention and chain-of-custody file custody |
